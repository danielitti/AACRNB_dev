view: sales_channel {
  derived_table: {
    ...
    FROM  (SELECT * FROM {{_user_attributes["commercial_road_new_business_schema_name"]}}.DIM_SALES_CHANNEL WHERE SALES_CHANNEL_LEVEL  IN ('0', '2')) LVL2
    LEFT JOIN
    (SELECT PARENT_SALES_CHANNEL_KEY, SALES_CHANNEL_KEY, SALES_CHANNEL_DESC AS LVL_1_SALES_CHANNEL_DESC , SALES_CHANNEL_FULL_DESC AS LVL_1_SALES_CHANNEL_FULL_DESC FROM {{_user_attributes["commercial_road_new_business_schema_name"]}}.DIM_SALES_CHANNEL WHERE SALES_CHANNEL_LEVEL IN ('1' ,'0')) LVL1
