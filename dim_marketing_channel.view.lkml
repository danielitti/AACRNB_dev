...
FROM  (SELECT * FROM {{_user_attributes["commercial_road_new_business_schema_name"]}}.DIM_SOURCE_CODE) SC
  LEFT JOIN
      (SELECT * FROM {{_user_attributes["commercial_road_new_business_schema_name"]}}.DIM_MARKETING_CHANNEL WHERE MARKETING_CHANNEL_LEVEL  IN ('0', '2')) LVL2
